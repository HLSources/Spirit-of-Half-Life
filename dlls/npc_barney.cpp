...
void CBarney :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case BARNEY_AE_RELOAD:
				if (pev->frags)
					EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/357_reload1.wav", 1, ATTN_NORM );
				else
					EMIT_SOUND( ENT(pev), CHAN_WEAPON, "items/9mmclip2.wav", 1, ATTN_NORM );
...
void CBarney :: Spawn()
{
	...
	if (pev->frags)
		m_cClipSize			= BARNEY_MAX_WEAPON_CLIP_357;
	else
		m_cClipSize			= BARNEY_MAX_WEAPON_CLIP_GLOCK;
